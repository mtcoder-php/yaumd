<template>
    <AppLayout title="Kitobni tahrirlash">
        <div class="max-w-2xl mx-auto space-y-5">

            <!-- Header -->
            <div class="flex items-center gap-4">
                <Link :href="route('admin.library.show', book.id)"
                ...
                <h1 class="text-xl font-bold text-gray-900">Kitobni tahrirlash</h1>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input v-model="form.title" type="text" class="field-input" :class="form.errors.title ? 'field-error' : ''">
                        <p v-if="form.errors.title" class="err">{{ form.errors.title }}</p>
                    </div>
                    <div>
                        <label class="field-label">Muallif <span class="req">*</span></label>
                        <input v-model="form.author" type="text" class="field-input" :class="form.errors.author ? 'field-error' : ''">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input v-model="form.publisher" type="text" class="field-input">
                    </div>
                    <div>
                        <label class="field-label">ISBN</label>
                        <input v-model="form.isbn" type="text" class="field-input" :class="form.errors.isbn ? 'field-error' : ''">
                    ...
                    <div>
                        ...
                        <input v-model.number="form.page_count" type="number" min="1" class="field-input">
                    </div>
                    <div>
                        <label class="field-label">Javon/qator manzili</label>
                        <input v-model="form.shelf_location" type="text" class="field-input">
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div v-if="form.access_type === 'paid'">
                        ...
                        <input v-model.number="form.price" type="number" min="0" step="1000"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" @click="$refs.fileInput.click()" class="btn-secondary">
                            ...
                            {{ book.has_digital_file ? 'Faylni almashtirish' : 'Fayl tanlash' }}
                        </button>
                        ...
                        <span v-else-if="book.has_digital_file" class="text-sm text-gray-500 flex items-center gap-1">
                            <Icon icon="mdi:file-check-outline" class="w-4 h-4 text-green-600" />
                            Fayl yuklangan
                        </span>
                    </div>
...
const form = useForm({
    ...
    published_year:  props.book.published_year ?? null,
    language:        props.book.language       || 'uz',
    description:     props.book.description    || '',
    cover_image:     null,
    page_count:      props.book.page_count     ?? null,
    shelf_location:  props.book.shelf_location || '',
    access_type:     props.book.access_type    || 'free',
...
const coverPreview = ref(props.book.cover_image_url || null)
