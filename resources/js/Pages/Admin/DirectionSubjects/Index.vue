            ...
            <div class="overflow-x-auto scrollbar-hide">
                <div class="flex items-center gap-2 w-max">
                    <button
                        ...
                        :key="faculty.id"
                        @click="activeFaculty = faculty.id"
                        class="px-4 py-2 text-sm font-medium rounded-xl border transition-all whitespace-nowrap"
                        :class="activeFaculty === faculty.id
                            ? 'border-[#0f3460] text-[#0f3460] bg-blue-50'
                            : 'border-gray-200 text-gray-500 hover:border-gray-300'"
                    >
                        {{ faculty.short_name || faculty.name_uz }}
                    </button>
                </div>
            </div>

            <!-- Yo'nalishlar -->
            <div v-if="activeFacultyData" class="space-y-4">
                <div
                    v-for="direction in activeFacultyData.directions"
                    :key="direction.id"
                ...
                >
                    ...
                    <div class="divide-y divide-gray-50">
                        <div v-if="!direction.subjects?.length" class="px-5 py-6 text-center text-gray-400">
                            <Icon icon="mdi:book-off-outline" class="w-8 h-8 mx-auto mb-2 opacity-40" />
                            <p class="text-xs">Hali fan biriktirilmagan</p>
                        </div>

                        <div
                            v-for="ds in direction.subjects"
                            :key="ds.id"
                        ...
                        >
                            <div class="flex items-center gap-3">
                                <!-- Blok badge -->
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold flex-shrink-0"
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="text-center">
                                    <p class="text-xs text-gray-400">Savollar</p>
                                    <p class="text-sm font-bold text-gray-700">{{ ds.questions_count }}</p>
                                </div>

                                <!-- Ball -->
                                <div class="text-center">
                                    <p class="text-xs text-gray-400">Ball/savol</p>
                                    <p class="text-sm font-bold" style="color:#0f3460">{{ ds.score_per_question }}</p>
                                </div>

                                <!-- Jami ball -->
                                <div class="text-center">
                                    <p class="text-xs text-gray-400">Jami</p>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select v-model="editForm.block_type" class="field-input">
                            <option v-for="bt in blockTypes" :key="bt.value" :value="bt.value">
                                {{ bt.label }}
                            </option>
                        </select>
                    </div>
        ...
        >
            <div class="bg-white rounded-2xl w-full max-w-sm p-6">
                ...
                <p class="text-sm text-gray-500 text-center mb-6">
                    <strong>{{ deleteTarget?.subject?.name_uz }}</strong> fanini yo'nalishdan olib tashlaysizmi?
                </p>
...
const activeFaculty    = ref(props.faculties[0]?.id || null)
...
const blockTypes = [
    {
        value: 'mandatory',
        label: 'Majburiy blok',
        desc:  'Barcha abituriyentlar uchun',
        defaultCount: 10,
        defaultScore: 1.1,
    },
    {
        value: 'specialty_1',
        label: '1-asosiy fan',
        desc:  'Mutaxassislik bloki (yuqori ball)',
        defaultCount: 30,
        defaultScore: 3.1,
    },
    {
        value: 'specialty_2',
        label: '2-asosiy fan',
        desc:  'Mutaxassislik bloki (o\'rta ball)',
        defaultCount: 30,
        defaultScore: 2.1,
    },
]

const activeFacultyData = computed(() =>
    props.faculties.find(f => f.id === activeFaculty.value)
)

const totalScore = (direction) => {
    if (!direction.subjects?.length) return 0
    return direction.subjects.reduce((sum, ds) => {
        return sum + (ds.questions_count * ds.score_per_question)
    }, 0).toFixed(1)
}

const blockLabel = (type) => {
    return blockTypes.find(b => b.value === type)?.label || type
}
